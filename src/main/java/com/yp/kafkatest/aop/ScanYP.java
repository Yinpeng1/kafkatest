...
import com.yp.kafkatest.controller.TestController;
...
public class ScanYP {
    ...
    public Set<Class<?>> getControllers() throws Exception{
        if (controllers == null) {
            ...
            Set<Class<?>> clsList = getClasses("com.yp.kafkatest.controller");
            if (clsList != null && clsList.size() > 0) {
                for (Class<?> cls : clsList) {
//                    if (cls.getAnnotation(YP.class) != null) {
...
//                    }
    ...
    public void setComings() throws Exception{
            Set<Class<?>> clsList = getClasses("com.yp.kafkatest.controller");
            if (clsList != null && clsList.size() > 0) {
                for (Class<?> cls : clsList) {
                    Field[] fields = cls.getDeclaredFields();
                    for (Field field: fields) {
                        if (field.getAnnotation(coming.class) != null){
...
//            try{
//                map.put(t, t.getClass().newInstance());
//            } catch (Exception e){
...
//    }
        ScanYP scan = new ScanYP();
        scan.getControllers().forEach(t -> {
            try{
                map.put(t, t.newInstance());
            } catch (Exception e){
                System.out.println("实例化失败");
            }
        });
        scan.setComings();
        TestController testController = (TestController) map.get(TestController.class);
        testController.getName(222);
    }
}
